class com.clubpenguin.net.Connection extends XMLSocket implements com.clubpenguin.net.IConnection
{
    ...
    {
        var _loc3 = {t: "sys"};
        var _loc2 = "<login z=\'" + zone + "\'><nick><![CDATA[" + name + "]]></nick><pword><![CDATA[" + pass + "]]></pword></login>";
    ...
    function sendXtMessage(xtName, cmdName, paramObj, type, roomId)
    {
        ...
        {
            var _loc5;
            _loc5 = rawProtocolSeparator + "xt" + rawProtocolSeparator + xtName + rawProtocolSeparator + cmdName + rawProtocolSeparator + roomId + rawProtocolSeparator;
            for (var _loc2 = 0; _loc2 < paramObj.length; ++_loc2)
            {
                _loc5 = _loc5 + (paramObj[_loc2].toString() + rawProtocolSeparator);
            } // end of for
    ...
    function connectionEstablished(ok)
    {
        if (ok)
        {
            ...
            preConnection = false;
            var _loc3 = {t: "sys"};
            var _loc2 = "<ver v=\'" + majVersion.toString() + minVersion.toString() + subVersion.toString() + "\' />";
